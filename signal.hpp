...
struct PlaneTFSF{
  ...
  __device__ PlaneTFSF(const ftype _t, const ftype _x, const ftype _y, const ftype _z) {
    ...
    if(dz!=FLT_MAX) phase = src.w*t-kx*(x-src.BoxMs)-ky*(y-src.BoxMa)-kz*(z-src.BoxPv);
...
struct DipoleTFSF{
  ...
  ftype3 E, B;
  static const ftype A=1.0;
  __device__ ftype EnvelopeTime(const ftype t) { return A*exp(-(t-src.t0)*(t-src.t0)/(src.tw*src.tw)); }
  __device__ ftype EnvelopeDiv1(const ftype t) { return A*exp(-(t-src.t0)*(t-src.t0)/(src.tw*src.tw))*(-2*(t-src.t0)/(src.tw*src.tw)); }
  __device__ ftype EnvelopeDiv2(const ftype t) { return A*exp(-(t-src.t0)*(t-src.t0)/(src.tw*src.tw))*(4*(t-src.t0)*(t-src.t0)/(src.tw*src.tw*src.tw*src.tw)) + A*expf(-(t-src.t0)*(t-src.t0)/(src.tw*src.tw))*(-2/(src.tw*src.tw)); }
  __device__ DipoleTFSF(const ftype _t, const ftype _x, const ftype _y, const ftype _z) {
    const ftype dxs=dx*0.5,dxa=dy*0.5,dxv=dz*0.5;
    ...
    ftype3 n = make_ftype3(x-Xdip,y-Ydip,z-Zdip);
    //ftype R = length(n);
    ftype R = sqrt(n.x*n.x+n.y*n.y+n.z*n.z);
    const ftype c=src.cSrc;
    phase = src.w*t-src.w/c*R;
    ftype3 d  = make_ftype3(0.0,0.0,    sin(phase)*0.5*(1-cos(phase*src.kEnv)));
