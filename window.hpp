...
/*    cuTimer t0; 
  ...
  inline void RAMexch(const int ixdev, const int ixhost) {
    ...
    for(int idev=0, iy=0; idev<NDev; iy+=NStripe[idev], idev++) {
      ...
      if(ixhost   <Np      && ixhost   >=0         ) CHECK_ERROR( cudaMemcpyAsync(&dataDisp [ixhost*Na+iy]                      , &parsHost.ragsDisp [idev][ixdev*NStripe[idev]], sizeof(DiamondRagDisp)*1*NStripe[idev], cudaMemcpyDeviceToHost, streamCopy) );
      if(ixhost-Ns<Np      && ixhost-Ns>=0         ) CHECK_ERROR( cudaMemcpyAsync(&parsHost.ragsDisp [idev][ixdev*NStripe[idev]], &dataDisp [(ixhost-Ns)*Na+iy]                 , sizeof(DiamondRagDisp)*1*NStripe[idev], cudaMemcpyHostToDevice, streamCopy) );
